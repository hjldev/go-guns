...
func (e *Menu) GetByRoleName(rolename string) (Menus []Menu, err error) {
	table := database.Db.Table(e.TableName()).Select("sys_menu.*").Joins("left join sys_role_menu on sys_role_menu.menu_id=sys_menu.menu_id")
...
func InitPaths(menu *Menu) (err error) {
	...
	if int(menu.ParentId) != 0 {
		database.Db.Table("sys_menu").Where("menu_id = ?", menu.ParentId).First(parentMenu)
		if parentMenu.Paths == "" {
	...
	database.Db.Table("sys_menu").Where("menu_id = ?", menu.MenuId).Update("paths", menu.Paths)
	return
}
...
func (e *Menu) Delete(id int) (success bool, err error) {
	if err = database.Db.Table(e.TableName()).Where("menu_id = ?", id).Delete(&Menu{}).Error; err != nil {
		success = false
		return
	}
	success = true
