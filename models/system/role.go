...
func (role *Role) GetPage(pageSize int, pageIndex int) ([]Role, int, error) {
	var doc []Role

	table := database.Db.Select("*").Table("sys_role")
	if role.RoleId != 0 {
...
func (role *Role) Get() (SysRole Role, err error) {
	table := database.Db.Table("sys_role")
	if role.RoleId != 0 {
		table = table.Where("role_id = ?", role.RoleId)
	}
	if role.RoleName != "" {
...
func (role *Role) GetList() (SysRole []Role, err error) {
	table := database.Db.Table("sys_role")
	if role.RoleId != 0 {
		table = table.Where("role_id = ?", role.RoleId)
	}
...
// 获取角色对应的菜单ids
func (role *Role) GetRoleMeunId() ([]int, error) {
	menuIds := make([]int, 0)
...
func (role *Role) Insert() (id int, err error) {
	var i int64 = 0
	database.Db.Table(role.TableName()).Where("role_name=? or role_key = ?", role.RoleName, role.RoleKey).Count(&i)
	if i > 0 {
		return 0, errors.New("角色名称或者角色标识已经存在！")
	}
	role.UpdateBy = ""
	result := database.Db.Table(role.TableName()).Create(&role)
	if result.Error != nil {
		err = result.Error
		return
	}
...
func (role *Role) GetRoleDeptId() ([]int, error) {
	deptIds := make([]int, 0)
...
//修改
func (role *Role) Update(id int) (update Role, err error) {
	if err = database.Db.Table(role.TableName()).First(&update, id).Error; err != nil {
		return
	}
	...
	//参数1:是要修改的数据
	//参数2:是修改的数据
	if err = database.Db.Table(role.TableName()).Model(&update).Updates(&role).Error; err != nil {
		return
	}
	return
}
...
//批量删除
func (role *Role) BatchDelete(id []int) (Result bool, err error) {
	if err = database.Db.Table(role.TableName()).Where("role_id in (?)", id).Delete(Role{}).Error; err != nil {
		return
	}
	Result = true
	return
